...
export function AssumptionsProvider({ children }: { children: React.ReactNode }) {
  const [assumptions, setAssumptionsState] = useState<Assumptions>(
    loadJson<Assumptions>(STORAGE_KEYS.assumptions, { ...defaults, displayCashflowsAs: 'current' })
  );
  const value = useMemo<AssumptionsContextValue>(() => ({
    ...assumptions,
    setAssumptions: (next) => setAssumptionsState((prev) => { const v = { ...prev, ...next }; saveJson(STORAGE_KEYS.assumptions, v); return v; }),
    resetAssumptions: () => setAssumptionsState(() => { const v = { ...(defaults as any), displayCashflowsAs: 'current' }; saveJson(STORAGE_KEYS.assumptions, v); return v; }),
  }), [assumptions]);
