...
import forecastDefaults from '../data/forecast.json';
...
export function Dashboard() {
  ...
  const [assets, setAssets] = React.useState<any[]>(loadJson(STORAGE_KEYS.networth, assetsLiabilities as any).assets ?? (assetsLiabilities.assets as any[]));
  const [liabilities, setLiabilities] = React.useState<any[]>(loadJson(STORAGE_KEYS.networth, assetsLiabilities as any).liabilities ?? (assetsLiabilities.liabilities as any[]));
  const assumptions = useAssumptions();
  const storedProgress = loadJson(STORAGE_KEYS.progress, progressData as any);
  const progressSeries = React.useMemo(() => toSeries((storedProgress.snapshots ?? progressData.snapshots) as any), [storedProgress]);
  const series = progressSeries;
  const currentNet = series[0]?.netWorth ?? 0;
  const latestNet = series[series.length - 1]?.netWorth ?? 0;
  const latest = latestSnapshot((storedProgress.snapshots ?? progressData.snapshots) as any);
  ...
  const storedForecast = loadJson(STORAGE_KEYS.forecast, forecastDefaults as any);
  const startNetWorth = latest ? (latest.assets - latest.liabilities) : (storedForecast.startNetWorth ?? 0);
  const computedForecastInputs = React.useMemo(() => ({
    ...storedForecast,
    startNetWorth,
    inflationRate: assumptions.inflationRate,
    defaultMonthlyReturn: assumptions.defaultMonthlyReturn,
  }), [storedForecast, startNetWorth, assumptions.inflationRate, assumptions.defaultMonthlyReturn]);
  const forecastSeries = React.useMemo(() => forecastNetWorth(computedForecastInputs, assumptions.planStartDate).points,
      ...
      <Grid>
        <Grid.Col span={{ base: 12, md: 8 }}>
          ...
          <Card withBorder mt="md">
            ...
            <LineChart h={260} data={forecastSeries} dataKey="date" series={[{ name: 'netWorth', label: 'Net Worth', color: 'indigo.5' }]} px="xs" curveType="linear" />
          </Card>
        </Grid.Col>
        <Grid.Col span={{ base: 12, md: 4 }}>
          <Stack>
            <AssetsList items={assets} onChange={(v) => { setAssets(v); saveJson(STORAGE_KEYS.networth, { assets: v, liabilities }); }} />
            <LiabilitiesList items={liabilities} onChange={(v) => { setLiabilities(v); saveJson(STORAGE_KEYS.networth, { assets, liabilities: v }); }} />
          </Stack>
        </Grid.Col>
      </Grid>
    </Stack>
