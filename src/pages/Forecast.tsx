...
export function Forecast() {
  const [inputs, setInputs] = React.useState<ForecastInputs>(loadJson(STORAGE_KEYS.forecast, defaults as any));
  ...
  const startNetWorth = latest ? (latest.assets - latest.liabilities) : inputs.startNetWorth;
  ...
  }), [inputs, startNetWorth, assumptions.inflationRate, assumptions.defaultMonthlyReturn]);

  const result = React.useMemo(() => forecastNetWorth(computed, assumptions.planStartDate), [computed, assumptions.planStartDate]);
      ...
      <Grid.Col span={{ base: 12, md: 5 }}>
        <Card withBorder>
          <Stack>
            ...
            <Group justify="space-between">
              <Text size="sm" c="dimmed">Monthly return</Text>
              <Text fw={600}>{(assumptions.defaultMonthlyReturn * 100).toFixed(2)}%</Text>
            </Group>
            <NumberInput label="Horizon (months)" value={inputs.horizonMonths} onChange={(v) => { const next = { ...inputs, horizonMonths: Number(v) || 0 }; setInputs(next); saveJson(STORAGE_KEYS.forecast, next); }} />
            <CashflowForm inputs={computed} onChange={(next) => { setInputs(next); saveJson(STORAGE_KEYS.forecast, next); }} />
          </Stack>
        </Card>
      </Grid.Col>
