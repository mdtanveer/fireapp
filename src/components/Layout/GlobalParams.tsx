...
export function GlobalParams() {
  ...
  return (
    <Group>
      ...
      >
        {(props) => (
          <Button size="xs" variant="filled" {...props}>
            Import JSON
          </Button>
        )}
      </FileButton>
      ...
      >
        <Popover.Target>
          <Button
            size="xs"
            variant="default"
            onClick={() => setOpenedDisp((o) => !o)}
          >
            {assumptions.displayCashflowsAs === "current"
              ? "Show: Current values"
              : "Show: Input values"}
          </Button>
        </Popover.Target>
        <Popover.Dropdown>
          <Stack>
            <SegmentedControl
              value={assumptions.displayCashflowsAs ?? "current"}
              onChange={(v) =>
                assumptions.setAssumptions({ displayCashflowsAs: v as any })
              }
              data={[
                { label: "Current (inflation-adjusted)", value: "current" },
                { label: "Input (as entered)", value: "input" },
              ]}
            />
          </Stack>
        </Popover.Dropdown>
